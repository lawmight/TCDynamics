...
class BugMonitor {
  ...
  async checkSecurity() {
    ...
    try {
      const auditOutput = execSync('npm audit --audit-level=moderate 2>&1', {
        encoding: 'utf8',
      })
  ...
  parseSecurityIssues(auditOutput) {
    ...
    lines.forEach(line => {
      if (
        line.includes('vulnerabilities found') ||
        line.includes('high') ||
...
*Auto-generated by Bug Monitor*
  ...
  async run() {
    ...
    this.bugs.forEach(bug => {
      ...
      report.bugsBySeverity[bug.severity] =
        (report.bugsBySeverity[bug.severity] || 0) + 1
    })
    ...
    this.log(
      `  - Critical bugs: ${this.criticalBugs.length}`,
      this.criticalBugs.length > 0 ? 'critical' : 'info'
    )
...
function main() {
  ...
  const options = {
    ...
    slackWebhook: args
      .find(arg => arg.startsWith('--slack-webhook='))
      ?.split('=')[1],
  }
