...
const validateConfig = () => {
  ...
  if (missingVars.length > 0) {
    ...
    console.error('Please check your .env file and ensure all required Azure service credentials are configured.');
    return false;
  }
...
export class AzureOpenAIService {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          throw new Error('Access forbidden. Please check your Azure OpenAI permissions.');
      ...
      const aiResponse = data.choices[0].message.content || "Désolé, je n'ai pas pu générer une réponse appropriée.";
      ...
      let errorMessage = "Je suis désolé, mais je rencontre un problème technique.";

      if (error instanceof Error) {
        if (error.message.includes('not properly configured')) {
          errorMessage = "Le service IA n'est pas configuré. Veuillez contacter le support.";
        } else if (error.message.includes('Authentication failed')) {
          errorMessage = "Erreur d'authentification avec le service IA.";
        } else if (error.message.includes('Rate limit')) {
          errorMessage = "Trop de demandes. Veuillez réessayer dans quelques instants.";
      ...
      return {
        response: errorMessage + " Veuillez réessayer dans quelques instants ou contacter notre support.",
        sessionId,
        timestamp: new Date().toISOString()
      };
    }
  ...
  private async storeConversation(sessionId: string, userMessage: string, aiResponse: string): Promise<void> {
    try {
      const conversation = {
...
export class AzureVisionService {
  ...
  constructor() {
    ...
    if (!this.endpoint || !this.key) {
      throw new Error('Azure Vision configuration is incomplete. Please check your environment variables.');
    }
  }

  async processDocument(imageData: string, fileName: string): Promise<DocumentProcessingResponse> {
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          throw new Error('Authentication failed. Please check your Azure Vision API key.');
        } else if (response.status === 403) {
          throw new Error('Access forbidden. Please check your Azure Vision permissions.');
        } else if (response.status === 429) {
          throw new Error('Rate limit exceeded. Please try again later.');
        } else if (response.status === 400) {
          throw new Error('Invalid image format or size. Please use a valid image file.');
        } else {
      ...
      if (!resultResponse.ok) {
        ...
        throw new Error(`Failed to get analysis results: ${resultResponse.status} - ${errorText}`);
      }

      const result = await resultResponse.json();

      if (result.status === 'succeeded') {
        const extractedText = result.analyzeResult?.readResults?.[0]?.lines
      ...
      if (error instanceof Error) {
        if (error.message.includes('not properly configured')) {
          errorMessage = 'Le service de reconnaissance d\'image n\'est pas configuré.';
        } else if (error.message.includes('Authentication failed')) {
          errorMessage = 'Erreur d\'authentification avec le service de reconnaissance d\'image.';
        } else if (error.message.includes('Rate limit')) {
          errorMessage = 'Trop de demandes. Veuillez réessayer dans quelques instants.';
        } else if (error.message.includes('Invalid image')) {
          errorMessage = 'Format d\'image invalide. Veuillez utiliser une image JPG, PNG ou PDF.';
        } else if (error.message.includes('network') || error.message.includes('fetch')) {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('Failed to fetch') || error.message.includes('network')) {
          ...
          errorMessage = 'Erreur de configuration CORS. Veuillez contacter le support.';
...
export const chatAPI = {
  sendMessage: (message: string, sessionId: string) => openAIService.sendMessage(message, sessionId)
};

export const visionAPI = {
  processDocument: (imageData: string, fileName: string) => visionService.processDocument(imageData, fileName)
};

export const contactAPI = {
  submitContactForm: (data: ContactFormData) => contactService.submitContactForm(data)
};
