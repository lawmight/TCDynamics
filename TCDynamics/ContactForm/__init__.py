...
def validate_contact_data(data: Dict[str, Any]) -> tuple[bool, str]:
    ...
    if len(data['message']) < 10:
        return False, "Le message doit contenir au moins 10 caractères"
    
    return True, ""

def create_contact_email(data: Dict[str, Any]) -> MIMEMultipart:
    """Create contact email template"""
    msg = MIMEMultipart('alternative')
    msg['Subject'] = f"Nouveau message de contact - {data['name']}"
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            <h2 style="color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px;">
                Nouveau message de contact - TCDynamics
            </h2>
            ...
            <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin: 20px 0;">
                ...
                {f"<p><strong>Entreprise:</strong> {data['company']}</p>" if data.get('company') else ""}
            </div>
            ...
            <div style="background-color: #e8f4f8; padding: 15px; border-radius: 5px; margin: 20px 0; font-size: 12px; color: #666;">
                ...
                <p><strong>Source:</strong> Formulaire de contact TCDynamics</p>
            </div>
        </div>
    </body>
    </html>
...
def send_email(msg: MIMEMultipart) -> bool:
    ...
    try:
        ...
        logging.info(f"Contact email sent successfully to {email_user}")
        return True
...
def contact_form(req: func.HttpRequest) -> func.HttpResponse:
    ...
    logging.info('Contact form function processed a request.')
    ...
    try:
        ...
        # Validate data
        is_valid, error_message = validate_contact_data(req_body)
        if not is_valid:
            return func.HttpResponse(
                json.dumps({
                    "success": False,
        ...
        email_msg = create_contact_email(req_body)
        ...
        if email_sent:
            return func.HttpResponse(
                json.dumps({
                    ...
                    "messageId": f"contact_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                }),
                status_code=200,
                headers={"Content-Type": "application/json"}
            )
