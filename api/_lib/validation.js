...
/**
 * Validation result structure: { valid: boolean, errors: string[], warnings: string[] }
 */
...
export const validateEmail = email => {
  ...
  if (
    disposableDomains.some(domain =>
      trimmedEmail.toLowerCase().includes(domain)
    )
  ) {
    warnings.push(
...
export const validateMessage = (
  ...
  const spamPatterns = [
    /http[s]?:\/\/[^\s]+/gi, // URLs
    /(?:\d{3}[-\.\s]??\d{2}[-\.\s]??\d{4}|\(\d{3}\)\s*\d{3}[-\.\s]??\d{4}|\d{3}\.\d{3}\.\d{4})/g, // Phone numbers
...
export const validationSchemas = {
  contact: {
    ...
    phone: validatePhone,
    company: value =>
      !value ? { valid: true, errors: [], warnings: [] } : validateName(value, 'Entreprise'),
  },
  ...
  demo: {
    ...
    timeline: value => {
      ...
      return validTimelines.includes(value)
        ? { valid: true, errors: [], warnings: [] }
...
export const validateFormData = (formData, schema) => {
  ...
  for (const [field, validator] of Object.entries(schemaDefinition)) {
    ...
    if (result.warnings.length > 0) {
      allWarnings.push(
        ...result.warnings.map(warning => `${field}: ${warning}`)
      )
    }
  }
