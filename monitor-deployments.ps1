...
$ghRuns = gh run list --branch simplify --limit 5 --json databaseId,status,conclusion,name,createdAt,url 2>$null | ConvertFrom-Json
...
if ($ghRuns -and $ghRuns.Count -gt 0) {
    ...
    Write-Host "WARNING: No GitHub Actions runs found for 'simplify' branch" -ForegroundColor Yellow
